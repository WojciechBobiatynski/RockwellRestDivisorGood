package pl.sodexo.it.gryf.web.ti.util;

public final class UrlConstants {

    public static final String USER_INFO_REST = "/rest/userinfo";
    public static final String TRAINING_RESERVATION_REST = "/rest/trainingreservation";

    public static final String PATH_REIMBURSEMENT_REST = "/rest/reimbursements";
    public static final String PATH_REIMBURSEMENT_LIST = "/list";
    public static final String PATH_REIMBURSEMENTS_STATUSES_LIST = "/statuses";

    public static final String PATH_TRAINING_TO_REIMBURSE_REST = "/rest/trainingToReimburse";
    public static final String PATH_TRAINING_TO_REIMBURSE_LIST = "/list";
    public static final String PATH_TRAINING_TO_REIMBURSE_STATUSES_LIST = "/statuses";
}
