<script src="${pageContext.request.contextPath}/js/components/gryf-config.js"></script>
<script src="${pageContext.request.contextPath}/js/dictionaries/gryf-dictionaries.js"></script>
<script src="${pageContext.request.contextPath}/js/dictionaries/zipCodes/zipCodesModel.js"></script>
<script src="${pageContext.request.contextPath}/js/publicbenefits/enterprises/gryf-enterprises.js"></script>
<script src="${pageContext.request.contextPath}/js/publicbenefits/enterprises/enterprisesServices.js"></script>
<script src="${pageContext.request.contextPath}/js/publicbenefits/grantapplications/gryf-grantApplications.js"></script>
<script src="${pageContext.request.contextPath}/js/publicbenefits/grantapplications/grantProgramServices.js"></script>
<script src="${pageContext.request.contextPath}/js/publicbenefits/grantapplications/grantApplicationsServices.js"></script>
<script src="${pageContext.request.contextPath}/js/publicbenefits/grantapplications/grantApplicationsControllers.js"></script>

<div ng-app="gryf.grantApplications" keypress-events>
    <div class="mainContent" ng-view></div>
</div>