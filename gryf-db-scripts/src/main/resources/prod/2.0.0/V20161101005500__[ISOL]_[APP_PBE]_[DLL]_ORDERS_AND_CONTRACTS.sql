...
-- Komentarze
COMMENT ON TABLE APP_PBE.ORDER_FLOWS_FOR_GR_PROGRAMS IS '@Author(Tomasz Bilski); @Project(GryfBonEl); @Date(2016-11-05);@Purpose(Tabel łącząca program dofinansowania i przepływ zamówienia);';
COMMENT ON COLUMN APP_PBE.ORDER_FLOWS_FOR_GR_PROGRAMS.ID IS 'Klucz główny';
COMMENT ON COLUMN APP_PBE.ORDER_FLOWS_FOR_GR_PROGRAMS.GRANT_PROGRAM_ID IS 'Indetyfikator programu dofinansowania';
COMMENT ON COLUMN APP_PBE.ORDER_FLOWS_FOR_GR_PROGRAMS.ORDER_FLOW_ID IS 'Indetyfikator przepływu zamówienia';
COMMENT ON COLUMN APP_PBE.ORDER_FLOWS_FOR_GR_PROGRAMS.DATE_FROM IS 'Data od której obowiązywuje dane połączenie';
COMMENT ON COLUMN APP_PBE.ORDER_FLOWS_FOR_GR_PROGRAMS.DATE_TO IS 'Data do której obowiązywuje dane połączenie';
