...
alter table APP_PBE.ORDERS drop column INDIVIDUAL_ID;
...
COMMENT ON COLUMN APP_PBE.ORDERS.EXTERNAL_ORDER_ID IS 'Identyfikator zewnetrzny zamówienia';
...
ALTER TABLE APP_PBE.ORDER_INVOICES ADD CONSTRAINT ORD_INV_INV_FK FOREIGN KEY (INVOICE_ID) REFERENCES APP_FIN.INVOICES (ID);



COMMENT ON TABLE APP_PBE.ORDER_INVOICES IS '@Author(Tomasz Bilski); @Project(GryfBonEl); @Date(2016-11-05);@Purpose(Tabel wartoœci faktur dla umów);';
COMMENT ON COLUMN APP_PBE.ORDER_INVOICES.ID IS 'Identyfikato klucz sztuczny';
COMMENT ON COLUMN APP_PBE.ORDER_INVOICES.ORDER_ID IS 'Identyfikator zamówienia';
...
COMMENT ON COLUMN APP_PBE.ORDER_INVOICES.INVOICE_TYPE IS 'Typ umowy';
...
COMMENT ON COLUMN APP_PBE.ORDER_INVOICES.CREATED_USER IS 'Login u¿ytkownika, który utworzy³ rekord';
COMMENT ON COLUMN APP_PBE.ORDER_INVOICES.CREATED_TIMESTAMP IS 'Data utworzenia rekordu';
COMMENT ON COLUMN APP_PBE.ORDER_INVOICES.MODIFIED_USER IS 'Login u¿ytkownika, który ostatnio modyfikowa³ rekord';
