MERGE INTO APP_PBE.GRANT_PROGRAM_PARAM_TYPES ug USING (
   SELECT
     ...
     'Liczba dni liczona od koñca szkolenia po której dane rozliczenie bonów elektronicznych, jeœli nie zosta³o rozliczone, uznawane jest za przeterminowane' DESCRIPRTION
   FROM dual
  ) ins
ON (ug.ID = ins.ID)
WHEN MATCHED THEN
